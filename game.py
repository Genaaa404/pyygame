...
def draw_menu(screen):
    ...
    title_text = font_title.render("OUTLAST PYTHON", True, WHITE)
    ...
    # Opción Jugar
    play_text = font_menu.render("JUGAR", True, WHITE)
    play_rect = play_text.get_rect(center=(WIDTH//2, HEIGHT//2))
    screen.blit(play_text, play_rect)
    
    # Opción Salir
    quit_text = font_menu.render("SALIR", True, WHITE)
    quit_rect = quit_text.get_rect(center=(WIDTH//2, HEIGHT//2 + 60))
    screen.blit(quit_text, quit_rect)
    
    # Instrucciones
    font_instructions = pygame.font.SysFont(None, 24)
    instructions = font_instructions.render("Usa las flechas ARRIBA/ABAJO y ENTER para seleccionar", True, GRAY)
...
def handle_menu(selected_option):
    """Maneja la lógica del menú"""
    keys = pygame.key.get_pressed()
    
    for event in pygame.event.get():
        if event.type == QUIT:
            return -1, selected_option  # -1 significa salir del juego
        if event.type == KEYDOWN:
            if event.key == K_UP:
                selected_option = 0  # Jugar
            elif event.key == K_DOWN:
                selected_option = 1  # Salir
            elif event.key == K_RETURN:
                if selected_option == 0:
                    return PLAYING, selected_option  # Comenzar el juego
                else:
                    return -1, selected_option  # Salir del juego
    
    return MENU, selected_option

def draw_menu_with_selection(screen, selected_option):
    ...
    # Opción Jugar
    play_color = GREEN if selected_option == 0 else WHITE
    play_text = font_menu.render("JUGAR", True, play_color)
    play_rect = play_text.get_rect(center=(WIDTH//2, HEIGHT//2))
    screen.blit(play_text, play_rect)
    
    # Flecha para opción seleccionada
    if selected_option == 0:
        arrow_text = font_menu.render(">", True, GREEN)
        screen.blit(arrow_text, (play_rect.left - 40, play_rect.top))
    
    # Opción Salir
    quit_color = GREEN if selected_option == 1 else WHITE
    quit_text = font_menu.render("SALIR", True, quit_color)
    quit_rect = quit_text.get_rect(center=(WIDTH//2, HEIGHT//2 + 60))
    screen.blit(quit_text, quit_rect)
    ...
    if selected_option == 1:
        arrow_text = font_menu.render(">", True, GREEN)
        screen.blit(arrow_text, (quit_rect.left - 40, quit_rect.top))
...
def main():
    ...
    # Variables del juego (se inicializarán cuando se comience a jugar)
    walls, hiding_spots, documents, batteries = None, None, None, None
    player = None
    enemies = None
    collected_docs = None
    collected_batteries = None
    exit_door = None
    ...
    while running:
        if game_state == MENU:
            # Mostrar menú
            draw_menu_with_selection(screen, selected_option)
            pygame.display.flip()
            
            # Manejar eventos del menú
            new_state, selected_option = handle_menu(selected_option)
            if new_state == -1:  # Salir del juego
                running = False
            elif new_state == PLAYING:  # Comenzar el juego
            ...
            text = font_small.render("Presiona ESC para volver al menú", True, WHITE)
            ...
            font = pygame.font.SysFont(None, 24)
            text = font.render("Presiona R para jugar de nuevo o ESC para volver al menú", True, WHITE)
            screen.blit(text, (WIDTH//2 - 250, HEIGHT//2 + 30))
